# Hi, I'm Raffael 👋
### Strategic Market Researcher | Data Science Enthusiast | Behavioural Analyst

I bridge the gap between **raw consumer data** and **executive strategy**. Based in Toronto, I specialise in identifying the "Say-Do" gap—the space where traditional market research often fails and behavioural analytics succeeds. 

By combining **Python-based data modelling** with **Behavioural Economics frameworks**, I help global brands navigate complex market shifts and optimise for long-term growth.

---

## 🚀 Core Competencies
* **Analytics:** Price Elasticity Modelling, Growth Attribution, Consumer Outflow Analysis.
* **Strategy:** Behavioural Economics (Habit Loops, Friction Analysis), "Jobs to be Done" (JTBD).
* **Tech Stack:** Python (Pandas, Scikit-learn), NLP for Qualitative Analysis, Advanced Data Visualisation.
* **Market Focus:** Beauty & Luxury, Prestige Skincare, Medical Cosmetology Synergy.

---

## 📂 Featured Projects

### 🧴 [Predictive Behavioural Modelling: Growth Recovery for ANR Serum](https://github.com/raffaelyg/Market-Analytics-ELC-ANR-Price-Elasticity)
**Client:** Estée Lauder China (ELC)  
**Objective:** Diagnose growth stagnation and model price elasticity across SKU sizes.
* **Key Insight:** Identified a specific 960元 "Optimal Price Peak" and exposed how aggressive promotions were "grooming" negative consumer habit loops.
* **Frameworks:** The "Say-Do" Gap, Habit Loop Analysis, Price-per-mL Friction.
* **Impact:** 7% projected revenue uplift through SKU discipline and functional re-positioning.


---

### 💄 [Strategic Market Diagnostic: Ultra-Premium & Luxury Skincare](https://github.com/raffaelyg/Market-Analytics-COTY-premium-buyers)
**Client:** Coty China  
**Objective:** Evaluate post-pandemic recovery and the impact of Medical Cosmetology on luxury skincare.
* **Key Insight:** Discovered that 1 in 8 females in Tier 1-4 China are Medical Cosmetology users, creating a high-growth "Post-Treatment Repair" functional white space.
* **Frameworks:** Market Polarisation, Synergy Mapping, Growth Attribution Waterfalls.
* **Impact:** Provided a roadmap for the portfolio to pivot toward "Medical-Grade Repair" narratives to capture high-value cohorts.



---

## 🎓 Education & Background
* **University of Cambridge**
  * **Certificate** Data Science with Machine Learning | 2025 - 2026 
* **Hult International Business School** | MSc.
  * **MSc.** International Business | 2019 - 2020
* **Western Univerisity** 
  * **BSc.** Biology | 2013 - 2017 
  * **PGDip.** Human Resources Management | 2017 - 2018 
  * **Extra:** President | Calligraphers of Western; VP Promotion & Performer | The Acapella Project
 


---

## 🛠️ Technical Toolkit
| Category | Tools & Platforms |
| :--- | :--- |
| **Data Manipulation** | `Python`, `Pandas`, `NumPy`, `SQL` |
| **Machine Learning** | `Scikit-learn` (Regression, Clustering), `NLTK` |
| **Visualisation** | `Matplotlib`, `Seaborn`, `PowerPoint` (Executive Storytelling) |
| **Methods** | `Price Elasticity`, `Retention Modelling`, `K-Means Clustering` |

---

## 📫 Connect with Me
* **Location:** Toronto, ON, Canada 🇨🇦
* **LinkedIn:** https://www.linkedin.com/in/raffaelyg/
* **Notion Portfolio:** https://raffaelyg.notion.site/portfolio/
